...
.. From the listing obtain the GUID for the selected service binding and execute the following command:
`curl cf curl /v3/service_bindings/[GUID] -X DELETE`. For example:
...
. Remove the app
.. For the task you want to delete,  obtain the href from the `self` attribute .
